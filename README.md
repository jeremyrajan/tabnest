...
- **Learns from manual moves:** when you move a website into a category group, future tabs from its base domain and subdomains use the category automatically. Existing manual category placements are imported when Tabnest starts; a unique majority resolves sites already placed in more than one category. Moving a site into a custom group sends future matching tabs to that exact group while it exists. Move the website again to teach a new destination.
